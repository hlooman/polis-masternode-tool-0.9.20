...
class ProposalsDlg(QDialog, ui_proposals.Ui_ProposalsDlg, wnd_utils.WndUtils):
    def __init__(self, parent, polisd_intf):
        ...
        for idx, mn in enumerate(self.main_wnd.config.masternodes):
            ...
            if mn_ident not in mn_idents:
                if polis_utils.privkey_valid(mn.privateKey):
    ...
    def display_budget_summary(self):
        ...
        message = '<html><head></head><style>td{padding-right:10px;white-space:nowrap;}</style><body>' \
                  ...
                  f'{bra}{budget_approved_user_yes}<tr><td><b>Budget available:</b> {app_utils.to_string(round(self.next_budget_amount))} Polis</td><td></td></tr>' \
                  f'</table></body></html>'
    ...
    def read_proposals_from_network(self):
        ...
        try:
            ...
            logging.info('Reading proposals from the Polis network.')
            ...
            if len(proposals_new) > 0:
                ...
                logging.warning('No proposals returned from polisd.')
            ...
            logging.exception('Exception wile reading proposals from Polis network.')
            self.display_message('')
    ...
    def read_governance_data(self):
        try:
            ...
            self.next_budget_amount = float(self.polisd_intf.getsuperblockbudget(sb_next))

            # superblocks occur every 16616 blocks (approximately 28.8 days)
            cur_block = self.polisd_intf.getblockcount()

            sb_last_hash = self.polisd_intf.getblockhash(sb_last)
            last_bh = self.polisd_intf.getblockheader(sb_last_hash)
            self.last_superblock_time = last_bh['time']
            self.next_superblock_time = 0
            if cur_block > 0 and cur_block <= sb_next:
                cur_hash = self.polisd_intf.getblockhash(cur_block)
                cur_bh = self.polisd_intf.getblockheader(cur_hash)
        ...
        try:
            ...
            if not self.polisd_intf.open():
                ...
                try:
                    ...
                    mns = self.polisd_intf.get_masternodelist('full')
    ...
    def read_external_attibutes(self, proposals):
        """Method reads additional proposal attributes from an external source such as PolisCentral.org
        ...
        try:
            url = self.main_wnd.config.polis_central_proposal_api
            if url:
                ...
                if not self.polisd_intf.open():
                    ...
                    try:
                        ...
                        for row_idx, prop in enumerate(proposals):
                            ...
                            tm_begin = time.time()
                            try:
                                votes = self.polisd_intf.gobject("getvotes", prop.get_value('hash'))
                            except Exception:
        ...
        for prop in proposal_list:
            ...
            for vote_idx, mn_info in enumerate(masternodes):
                ...
                try:
                    ...
                    step = 2
                    vote_sig = polis_utils.ecdsa_sign(serialize_for_sig, mn_info.masternode_config.privateKey,
                    ...
                    if step in (1, 4):
                        ...
                        logging.info('masternode_pub_key: %s' %
                                     str(polis_utils.privkey_to_pubkey(mn_info.masternode_config.privateKey)))
                        logging.info('masternode_pub_key_hash: %s' %
                                     str(polis_utils.pubkey_to_address(polis_utils.privkey_to_pubkey(
                                         mn_info.masternode_config.privateKey), self.app_config.polis_network)))
