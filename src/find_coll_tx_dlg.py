...
class FindCollateralTxDlg(QDialog, ui_find_coll_tx_dlg.Ui_FindCollateralTxDlg, wnd_utils.WndUtils):
    ...
    def setupUi(self):
        try:
            ...
            self.edtAddress.setText(self.polis_address)
    ...
    def display_utxos(self):
        ...
        if len(self.utxos):
            ...
            self.setBaseSize(sh)
            if self.read_only:
                msg = f'<span style="color:blue">Found 1000 Polis transaction(s):' \
                      f'</span>'
            else:
                msg = f'<span style="color:blue">Found 1000 Polis transaction(s). Click the "Apply" button to copy' \
            ...
            self.lblMessage.setText('<span style="color:red">Found no unspent 1000 Polis transactions  '
    ...
    def load_utxos_thread(self, ctrl):
        try:
            if not self.polisd_intf.open():
                ...
                try:
                    self.block_count = self.polisd_intf.getblockcount()
                    self.utxos = self.polisd_intf.getaddressutxos([self.polis_address])
