...
class PaymentTableModel(QAbstractTableModel):
    def __init__(self, parent, hide_collaterals_utxos, parent_wnd):
        ...
        self.columns = [
            ...
            ('satoshis', 'Amount (Polis)', True, 100),
...
UtxoSrcAddr = Tuple[str, str]  # Polis address (in HW) to be scanned for UTXOs (address, bip32 path)
...
class WalletDlg(QDialog, ui_send_payout_dlg.Ui_SendPayoutDlg, WndUtils):
    ...
    def restore_cache_settings(self):
        ...
        path = app_cache.get_value(CACHE_ITEM_HW_SRC_BIP32_PATH.replace('%NETWORK%', self.app_config.polis_network),
                                   polis_utils.get_default_bip32_path(self.app_config.polis_network), str)
        if polis_utils.validate_bip32_path(path):
            self.hw_src_bip32_path = path
        else:
            self.hw_src_bip32_path = polis_utils.get_default_bip32_path(self.app_config.polis_network)
        ...
        sel_nasternode = app_cache.get_value(
            CACHE_ITEM_UTXO_SRC_MASTRNODE.replace('%NETWORK%', self.app_config.polis_network), '', str)
        if sel_nasternode:
    ...
    def save_cache_settings(self):
        ...
        app_cache.set_value(CACHE_ITEM_HW_SRC_BIP32_PATH.replace('%NETWORK%', self.app_config.polis_network),
    ...
    def on_lbl_hw_account_base_path_linkActivated(self, text):
        ...
        if ok:
            try:
                polis_utils.bip32_path_string_to_n(path)
        ...
        if len(utxos):
            ...
            for utxo_idx, utxo in enumerate(utxos):
                ...
                if addr_hw != utxo['address']:
                    self.errorMsg("<html style=\"font-weight:normal\">Polis address inconsistency between UTXO (%d) and HW path: %s.<br><br>"
                                 "<b>HW address</b>: %s<br>"
            ...
            try:
                ...
                if dest_data:
                    ...
                    if len(tx_hex) > 90000:
                        ...
                        tx_dlg = TransactionDlg(self, self.main_ui.config, self.polisd_intf, tx_hex, use_is)
                        if tx_dlg.exec_():
        ...
        def apply_utxos(new_utxos: List[dict], existing_utxos: List[dict]):
            ...
            if self.utxo_src_mode == 1:
                if self.mn_src_index is not None and self.mn_src_index < len(self.masternode_addresses) and \
                        self.mn_src_index >= 0:
                    msg = f'<b>Balance:</b> {self.sel_addresses_balance} Polis&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} Polis&nbsp;&nbsp;&nbsp;<b>Address:</b> {self.masternode_addresses[self.mn_src_index][0]}'
                    self.set_message_2(msg)
                else:
                    msg = f'<b>Balance:</b> {self.sel_addresses_balance} Polis&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} Polis'
                    self.set_message_2(msg)
            elif self.utxo_src_mode == 2:
                msg = f'<b>Balance:</b> {self.sel_addresses_balance} Polis&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} Polis'
                self.set_message_2(msg)
            elif self.utxo_src_mode in (1, 3):
                msg = f'<b>Balance:</b> {self.sel_addresses_balance} Polis&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} Polis&nbsp;&nbsp;&nbsp;<b>Address:</b> {self.hw_src_address}'
                self.set_message_2(msg)
    ...
    def load_utxos_thread(self, ctrl: CtrlObject, new_utxos_out, existing_utxos_out):
        ...
        def get_addresses_to_scan(self, thread_ctrl: CtrlObject, addr_scan_ctrl: dict):
            ...
            try:
                if self.utxo_src_mode == 1:
                    ...
                    addr_count = 0
                    addr_n = polis_utils.bip32_path_string_to_n(self.hw_account_base_bip32_path)
                    ...
                    try:
                        ...
                        while True:
                            ...
                            for nr in range(1000):
                                ...
                                bip32_path = polis_utils.bip32_path_n_to_string(bip32_path_n)
                    ...
                    try:
                        ...
                        bip32_path_n = polis_utils.bip32_path_string_to_n(self.hw_src_bip32_path)
                        cur_addr = hw_intf.get_address_ext(self.main_ui.hw_session, bip32_path_n, db_cur,
                                                           self.app_config.hw_encrypt_string,
                                                           self.app_config.hw_decrypt_string)
        ...
        if not self.polisd_intf.open():
            ...
            tm_begin = time.time()
            try:
                cur_block_height = self.polisd_intf.getblockcount()
                ...
                for addr_path_chunk in get_addresses_to_scan(self, ctrl, addr_scan_ctrl):
                    try:
                        ...
                        balance = self.polisd_intf.getaddressbalance(addr_chunk)
                        ...
                        if balance.get('balance') > 0:
                            ...
                            uxs = self.polisd_intf.getaddressutxos(addr_chunk)
                            utxos_count += len(uxs)
                            ...
                            for idx, utxo in enumerate(uxs):
                                ...
                                if not cached_utxo:
                                    blockhash = self.polisd_intf.getblockhash(utxo.get('height'))
                                    bh = self.polisd_intf.getblockheader(blockhash)
                                    utxo['key'] = utxo_key
                                    ...
                                    try:
                                        ...
                                        rawtx = self.polisd_intf.getrawtransaction(utxo.get('txid'), 1)
                                        if rawtx:
