...
class SendFundsDestinationItem(QObject):
    ...
    def validate(self):
        ...
        if not address:
            valid = False
        elif not polis_utils.validate_address(address, self.app_config.polis_network):
            valid = False
        else:
            self.message = None
        if valid:
...
class SendFundsDestination(QtWidgets.QWidget, WndUtils):
    ...
    def setupUi(self, Form):
        ...
        # label dedicated to the second-unit value (e.g percentage if the main unit is set to (Polis) amount value)
    ...
    def update_the_change_ui(self):
        ...
        if self.values_unit == OUTPUT_VALUE_UNIT_AMOUNT:
            ...
            # pct
            the_change_second_unit_str = app_utils.to_string(self.change_amount) + ' Polis'
            if self.inputs_total_amount - self.fee_amount - self.add_to_fee > 0:
    ...
    def display_totals(self):
        ...
        text = f'<span class="label"><b>Total value of selected inputs:</b>&nbsp;</span><span class="value">&nbsp;{self.inputs_total_amount} Polis&nbsp;</span>'
