...
class AppConfig(object):
    def __init__(self):
        ...
        # runtime information, set after connecting to hardware wallet device; for Polis mainnet the value is
        # 'Polis', for Polis testnet, the value is 'Polis Testnet' or 'tPolis' depending on the (custom) firmware
        # used
        self.hw_coin_name = 'Polis'
    ...
    def init(self, app_path):
        ...
        cache_file_name = os.path.join(self.cache_dir, 'pmt_cache.json')
    ...
    def configure_cache(self):
        if self.is_testnet():
            db_cache_file_name = 'pmt_cache_testnet.db'
        else:
            db_cache_file_name = 'pmt_cache.db'
    ...
    def read_from_file(self, hw_session: HwSessionInfo, file_name: Optional[str] = None,
                ...
                for section in config.sections():
                    if re.match('MN\d', section):
                        ...
                        if correct_public_nodes:
                            if cfg.host.lower() == 'alice.polis-pmt.eu':
                                cfg.host = 'alice.polis-masternode-tool.org'
                                cfg.port = '443'
                                configuration_corrected = True
                            elif cfg.host.lower() == 'luna.polis-pmt.eu':
                                cfg.host = 'luna.polis-masternode-tool.org'
                                cfg.port = '443'
                                configuration_corrected = True
        ...
        try:
            cfgs = self.decode_connections(default_config.polisd_default_connections)
            if cfgs:
