...
class MainWindow(QMainWindow, WndUtils, ui_main_dlg.Ui_MainWindow):
    ...
    def setupUi(self):
        ...
        self.polisd_intf.initialize(self.config)
    ...
    def load_configuration_from_file(self, file_name) -> None:
        ...
        try:
            self.disconnect_hardware_wallet()
            polis_network_sav = self.config.polis_network
            ...
            if file_name:
                ...
                self.update_config_files_mru_menu_items()
                if polis_network_sav != self.config.polis_network:
                    self.disconnect_hardware_wallet()
            self.display_window_title()
    ...
    def display_window_title(self):
        ...
        if self.config.polis_network == 'TESTNET':
    ...
    def on_action_open_settings_window_triggered(self):
        ...
        if res and dlg.get_is_modified():
            ...
            self.polisd_intf.reload_configuration()
            if polis_network_sav != self.config.polis_network or hw_type_sav != self.config.hw_type:
                self.disconnect_hardware_wallet()
            self.display_window_title()
            self.update_edit_controls_state()
            if self.remote_app_params:
    ...
    def show_connection_initiated(self):
        ...
        self.setStatus1Text('<b>RPC network status:</b> trying %s...' % self.polisd_intf.get_active_conn_description(), 'black')
    ...
    def show_connection_failed(self):
        ...
        self.setStatus1Text('<b>RPC network status:</b> failed connection to %s' % self.polisd_intf.get_active_conn_description(), 'red')

    def show_connection_successful(self):
        """Shows status information after successful connetion to a Polis RPC node."""
        self.setStatus1Text('<b>RPC network status:</b> OK (%s)' % self.polisd_intf.get_active_conn_description(), 'green')
    ...
    def checkPolisdConnection(self, wait_for_check_finish=False, call_on_check_finished=None):
        """
        ...
        def wait_for_synch_finished_thread(ctrl):
            ...
            try:
                ...
                while not ctrl.finish:
                    ...
                    self.setMessage('Polisd is synchronizing: AssetID: %s, AssetName: %s' %
    ...
    def on_action_check_network_connection_triggered(self):
        ...
        if self.config.is_config_complete():
            ...
            # disable all actions that utilize polis network
            ...
            self.checkPolisdConnection(call_on_check_finished=connection_test_finished)
        else:
                    ...
                    if self.config.polis_network == 'TESTNET':
                        ...
                        if self.config.hw_type in (HWType.trezor, HWType.keepkey):
                            for coin in self.hw_client.features.coins:
                                if coin.coin_name.upper() == 'POLIS TESTNET' or coin.coin_shortcut.upper() == 'TPOLIS':
                                    found_testnet_support = True
                                    self.config.hw_coin_name = coin.coin_name
                                    break
                        elif self.config.hw_type == HWType.ledger_nano_s:
                            addr = hw_intf.get_address(self.hw_session,
                                                       polis_utils.get_default_bip32_path(self.config.polis_network))
                            if polis_utils.validate_address(addr, self.config.polis_network):
                                found_testnet_support = False
                        ...
                        self.config.hw_coin_name = 'Polis'
    ...
    def scan_hw_for_bip32_paths(self, addresses) -> Tuple[Dict[str, str], bool]:
        """
        Scans hardware wallet for bip32 paths of all Polis addresses passed in the addresses list.
        :param addresses: list of Polis addresses to scan
        :return: Tuple[Dict[str <polis address>, str <bip32 path>], bool <True if user cancelled scanning>]
        """
        paths_found = []
        ...
        def scan_for_bip32_thread(ctrl, addresses):
            ...
            for a bip32 paths with given Polis addresses.
            :param cfg: Thread dialog configuration object.
            :param addresses: list of Polis addresses to find bip32 path
            :return:
            """
            ...
            if self.hw_client:
                ...
                address_n = polis_utils.get_default_bip32_base_path_n(self.config.polis_network) + [None, 0, None]
    ...
    def update_ui_default_protocol(self):
        ...
        prot = None
        if self.remote_app_params:
            dp = self.remote_app_params.get('defaultPolisdProtocol')
            if dp:
                prot = str(dp.get(self.config.polis_network.lower()))
    ...
    def generate_mn_priv_key(self, compressed: bool):
        ...
        wif = polis_utils.generate_privkey(self.config.polis_network, compressed=compressed)
    ...
    def on_btnHwBip32ToAddress_clicked(self):
        """
        Convert BIP32 path to Polis address.
        :return:
        """
        try:
            self.connect_hardware_wallet()
            ...
            if self.curMasternode and self.curMasternode.collateralBip32Path:
                polis_addr = hw_intf.get_address(self.hw_session, self.curMasternode.collateralBip32Path)
                self.edtMnCollateralAddress.setText(polis_addr)
                self.curMasternode.collateralAddress = polis_addr
                self.curMnModified()
        except HardwareWalletCancelException:
            if self.hw_client:
                self.hw_client.init_device()
    ...
    def on_btnHwAddressToBip32_clicked(self):
        """
        Converts Polis address to BIP32 path, using hardware wallet.
        :return:
        """
            ...
            if self.curMasternode and self.curMasternode.collateralAddress:
                ...
                if not user_cancelled:
                    if not paths or len(paths) == 0:
                        self.errorMsg("Couldn't find Polis address in your hardware wallet. If you are using HW passphrase, "
    ...
    def get_default_protocol(self) -> int:
        ...
        if self.remote_app_params:
            dp = self.remote_app_params.get('defaultPolisdProtocol')
            if dp:
                prot = dp.get(self.config.polis_network.lower())
        return prot

    def create_mn_broadcast_msg(self, mn_protocol_version: int, ping_block_hash: str, masternode: MasternodeConfig,
                                sig_time: int = None) \
            -> polis_utils.CMasternodeBroadcast:

        if not sig_time:
            sig_time = int(time.time())
        mn_privkey = polis_utils.wif_to_privkey(masternode.privateKey, self.config.polis_network)
        if not mn_privkey:
        ...
        mn_broadcast = polis_utils.CMasternodeBroadcast(
        ...
        signature = mn_broadcast.sign_message(masternode.collateralBip32Path, hw_intf.hw_sign_message, self.hw_session,
                                              masternode.privateKey, self.config.polis_network)
    ...
    def on_btnBroadcastMn_clicked(self):
        """
        Broadcasts information about configured Masternode within Polis network using Hwrdware Wallet for signing message
        and a Polis daemon for relaying message.
        Building broadcast message is based on work of chaeplin (https://github.com/chaeplin/polismnb)
        """
        if self.curMasternode:
        ...
        self.checkPolisdConnection(wait_for_check_finish=True)
        if not self.polisd_connection_ok:
            self.errorMsg("Connection to Polis daemon is not established.")
            return
        if self.is_polisd_syncing:
            self.warnMsg("Polis daemon to which you are connected is synchronizing. You have to wait "
                         "until it's finished.")
            return
        ...
        try:
            mn_privkey = polis_utils.wif_to_privkey(self.curMasternode.privateKey, self.config.polis_network)
            if not mn_privkey:
            ...
            block_count = self.polisd_intf.getblockcount()
            block_hash = self.polisd_intf.getblockhash(block_count - 12)
            ...
            if not cfg_collateral_address:
                ...
                if self.queryDlg(message="The Polis address retrieved from the hardware wallet (%s) for the configured "
            ...
            try:
                ...
                if found:
                    if utxo.get('satoshis', None) != 100000000000:
                        if self.queryDlg(
                                message="Collateral transaction output should equal 100000000000 Satoshis (1000 Polis)"
            ...
            info = self.polisd_intf.getinfo(verify_node=True)
            ...
            if ret['overall'].startswith('Successfully decoded broadcast messages for 1 masternodes'):
                ...
                if failed_count == 0:
                    ...
                    self.errorMsg('Failed to start masternode.\n\nResponse from Polis daemon: %s.' % errorMessage)
            else:
    ...
    def get_masternode_status(self, masternode):
        ...
        if self.polisd_connection_ok:
            ...
            mns_info = self.polisd_intf.get_masternodelist('full', collateral_id)
            if len(mns_info):
    ...
    def get_masternode_status_description(self):
        ...
        if self.polisd_connection_ok:
            ...
            mns_info = self.polisd_intf.get_masternodelist('full', data_max_age=30)  # read new data from the network
                                                                                    # every 30 seconds
            ...
            if mn_info:
                ...
                if mn_info.status == 'ENABLED' or mn_info.status == 'PRE_ENABLED':
                    ...
                    color = 'red'
                enabled_mns_count = len(self.polisd_intf.payment_queue)
                ...
                if addr:
                    try:
                        bal = self.polisd_intf.getaddressbalance([addr])
                        if bal:
    ...
    def on_btnRefreshMnStatus_clicked(self):
        ...
        self.checkPolisdConnection(wait_for_check_finish=True, call_on_check_finished=enable_buttons)
        if self.polisd_connection_ok:
            try:
    ...
    def on_btnFindCollateral_clicked(self):
        """
        Open dialog with list of utxos of collateral polis address.
        :return:
        """
        if self.curMasternode and self.curMasternode.collateralAddress:
            ui = FindCollateralTxDlg(self, self.polisd_intf, self.curMasternode.collateralAddress,
